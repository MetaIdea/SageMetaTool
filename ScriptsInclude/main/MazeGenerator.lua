...
function CreateMaze(objectType, width, height, startX, startY)
	...
	for y = 1,height*2+1,1 do
		for x = 1,width*2+1,1 do
			if maze[y][x] then
				...
				ExecuteAction("UNIT_SPAWN_NAMED_LOCATION_ORIENTATION", GetUnitName(), objectType, CivilianTeam, {startX+x*20, startY+y*20, GetHeightAtPosition(startX+x*20, startY+y*20)}, 0)
			else
				MazeString = MazeString .. " "
			end
		end
	...
	ExecuteAction("PLAYER_SET_BASE_POWER", CivilianPlayer, 1000000)
...
if SAGE_GAME <= CNCZH then
	CreateMaze("AmericaPatriotBattery", 60, 60, MapSizeX, MapSizeY)
elseif SAGE_GAME <= BFME2ROTWK then
	CreateMaze("GondorSentryTower", 60, 60, MapSizeX, MapSizeY)
elseif SAGE_GAME <= CNC3KW then
	CreateMaze("ASPSculp03", 60, 60, MapSizeX, MapSizeY)
elseif SAGE_GAME <= CNCRA3U then
	CreateMaze("AlliedBaseDefense", 60, 60, MapSizeX, MapSizeY)
end
